...
func (s *OsdCsiServer) ValidateVolumeCapabilities(
	...
	volumes, err := s.driver.Inspect(context.Background(), []string{id}, &api.VolumeInspectOptions{})
...
func (s *OsdCsiServer) DeleteVolume(
	...
	volumes, err := s.driver.Inspect(context.Background(), []string{req.GetVolumeId()}, &api.VolumeInspectOptions{})
	if (err == nil && len(volumes) == 0) ||
...
func (s *OsdCsiServer) DeleteSnapshot(
	...
	volumes, err := s.driver.Inspect(context.Background(), []string{req.GetSnapshotId()}, &api.VolumeInspectOptions{})
	if (err == nil && len(volumes) == 0) ||
		(err != nil && err == kvdb.ErrNotFound) {
